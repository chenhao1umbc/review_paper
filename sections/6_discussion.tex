...
\begin{figure}[t]
  ...
  \includegraphics[width=\linewidth]{figures/trl_readiness.pdf}
...
\begin{table*}[t]
  ...
  \caption{Humanoid Clinical Deployment Readiness (HCDR) Framework (March 2026).}
  \label{tab:hcdr}
  {\fontsize{8.9pt}{10.2pt}\selectfont
  \resizebox{\linewidth}{!}{%
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{4.20cm}
    ...
    \toprule
    \textbf{Role} & \textbf{Autonomy} & \textbf{Patient Contact} & \textbf{Regulatory Complexity} & \textbf{Binding Constraint} \\
    \midrule
    ...
    \bottomrule
    \multicolumn{5}{@{}p{18.50cm}@{}}{\textbf{Binding constraint} is the HCDR axis whose gap most delays deployment for the role.} \\
    \multicolumn{5}{@{}p{18.50cm}@{}}{\textbf{TRL} = Technology Readiness Level (ISO 16290:2013); \textbf{RCM} = remote center of motion.} \\
  \end{tabular}
  }
  }
\end{table*}
...
clinical platforms (RHP Friends: purpose built but academic prototype; Sanctuary
Phoenix Gen~8: highest haptic sensing specification but closed ecosystem). These
platform specifications and their sources are compared in
Table~\ref{tab:platforms} and Section~\ref{sec:manipmaturity}. No
...
\begin{figure}[htbp]
  ...
  \includegraphics[width=0.85\columnwidth]{figures/regulatory_pathways-1.pdf}
...
\end{figure}
The regulatory instruments named here (the FDA De~Novo route, the absence of a
510(k) predicate, EU MDR classification and its Article~62 clinical investigation
requirement, and the EU AI Act application dates) are detailed with their official
sources in Section~\ref{sec:regulatory} and Table~\ref{tab:regulatory}. Building on
that analysis, the path to the first regulatory clearance requires three
sequential steps.
...
feels cared for, not processed. The uncanny valley phenomenon, the
