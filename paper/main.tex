...
%%----------------------------------------------------------------------
\begin{abstract}
Humanoid robots capable of dexterous bimanual manipulation and natural
language instruction are commercially available, yet no full-size bipedal
humanoid holds FDA clearance or CE marking for any clinical application. This
survey covers the 2022--2026 humanoid healthcare literature: 14 qualifying
papers identified, 25 platforms assessed, regulatory pathways analyzed, and
ISO~16290:2013 TRL criteria assigned to five clinical domains. All domains are
at TRL~3--4. Three roles---teleoperated clinical assistant, rehabilitation
demonstrator, and eldercare companion---have the strongest path to TRL~5. The
binding barrier is the lack of accepted bipedal gait safety standards for
patient-proximate environments.
\end{abstract}
...
sphenoidectomy~\cite{cho2026humanoid}. Not one of these systems holds FDA
clearance or CE marking for any clinical application. No registered clinical
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}llrllllll@{}}
    ...
    \midrule
    Figure 02        & Commercial & 168 & 28             & 20--25\,kg & Yes (in-house VLA)    & No      & None \\
    ...
    NAO\textsuperscript{*} & Academic & 58 & 25         & N/A   & No                    & Partial & HRI/ASD trials (social only) \\
...
robot serving as a physical proxy. This teleoperation-first architecture may
...
external researchers. Academic platforms---RHP Friends (30~DOF, purpose-built
for nursing), iCub3 (54~DOF, full-body tactile skin, 40+ units in research
...
10,000~hours of real robot demonstrations, with state-of-the-art results on ALOHA
...
The 14 papers identified in the systematic search (Section~\ref{sec:intro})
constitute a thin but internally consistent evidence base. Three cross-cutting
patterns define the literature: an almost exclusive reliance on a single
commercial platform (Unitree G1), concentration of study production in two
research clusters (UC San Diego Yip Laboratory; Johns Hopkins University), and a
uniform methodological limitation---no study involves patient subjects in primary
contact with a full-size bipedal humanoid. The analysis below leads with
cross-study findings within each domain and ends with a critical assessment of
study quality and generalizability.

\subsection{Clinical Procedures and Surgical Assistance}
\label{sec:procedures}

\textbf{Cross-study finding.} Teleoperation---not autonomous task execution---is
the viable near-term architecture for humanoid clinical procedures. Three studies
converge on this conclusion, all using the Unitree G1 as the physical proxy under
remote human cognitive and motor control~\cite{atar2025humanoids,liang2025lapsurgie,cho2026humanoid}.
Task success across seven clinical procedures reached approximately 70\% with
non-clinician teleoperators~\cite{atar2025humanoids}; inverse-kinematics
remapping enabled laparoscopic instrument manipulation with enforced
remote-center-of-motion constraint~\cite{liang2025lapsurgie}; and cadaveric
endoscopic surgery was performed with a humanoid as first assistant for the first
time on record~\cite{cho2026humanoid}. Across these three studies, the robot's
contribution is physical presence and dexterity; the clinical judgment and
real-time adaptation are supplied by the remote operator. Tasks requiring high
force precision (IV catheter insertion, finger-stick testing) showed the highest
failure rates, attributable to actuator force resolution and insufficient tactile
feedback~\cite{atar2025humanoids}. Tasks relying on gross positioning (ultrasound
probe placement, endoscope holding) showed stronger performance.

\textbf{Critical quality assessment.} The procedure evidence is methodologically
early-stage. No study involves patient subjects; the earliest data (Atar et al.)
uses test fixtures and mannequins. Two of three papers originate from a single
laboratory under a single PI (Michael Yip, UCSD), and all three use the Unitree
G1. Independent replication on alternative platforms has not occurred. No
standardized clinical outcome metric (procedure completion time, complication
rate, force safety margin) has been defined or applied across studies. The cadaver
study~\cite{cho2026humanoid} represents the highest pre-clinical evidence level
(analogous to bench-to-animal study transition), and identifies three specific
engineering targets for next-stage translation: endoscope stability, compliant
force control during anatomical contact, and autonomous scope repositioning. TRL
assessment: 3--4, uniformly across the three studies.

\subsection{Elderly and Nursing Care}
\label{sec:eldercare}

\textbf{Cross-study finding.} The eldercare evidence base is divided between
hardware demonstrations with zero patient contact and perception/design studies
that do not involve functional humanoid hardware. Benallegue et
al.~\cite{benallegue2025rhp} document the only bipedal humanoid designed with
nursing as its explicit primary application (RHP Friends), demonstrating
transparent autonomous-to-teleoperated handoff at IREX 2023---a clinically
relevant architectural principle with no equivalent in commercial platforms.
Institutional surveys of 269 US nursing home administrators
(Imtiaz and Khan~\cite{imtiaz2024perceptions}) identify the primary deployment
barriers as economic and evidence-based (cost, lack of effectiveness data) rather
than technical, a finding that reframes the problem: the bottleneck for eldercare
deployment is not engineering capability but regulatory and health-economic
evidence. The SPRING project~\cite{alamedapineda2024spring} provides the only
large-scale deployment data in an eldercare facility (60+ participants over 4
years), but on a wheeled non-bipedal platform (ARI); participants' preference for
upright humanoid form factor for dignified eye-level interaction establishes the
user acceptance hypothesis that bipedal systems must replicate with their own
evidence.

\textbf{Critical quality assessment.} No qualifying paper involves patient contact
with a full-size bipedal humanoid in an eldercare context. The user-centered
design study (Ghosh et al.~\cite{ghosh2024usercentered}, n\,=\,14) is
methodologically sound but uses NAO (58~cm, out of scope for physical care tasks).
The bipedal hardware evidence (RHP Friends) is a controlled demonstration, not a
clinical study. Generalizability is accordingly low; the perception and
institutional survey findings are more transferable than the hardware results.
TRL~4 for demonstrated hardware; TRL 4--5 for wheeled platform deployment evidence
(scope-constrained).

\subsection{Rehabilitation}
\label{sec:rehabilitation}

The central engineering gap in humanoid rehabilitation is contact safety:
without a validated compliance framework that bounds peak forces during
patient-proximate movement, no study involving physical contact between a
full-size bipedal humanoid and a patient is ethically or regulatorily
feasible. Lu et al.~\cite{lu2025gentlehumanoid} address this directly,
demonstrating measurable peak force reduction during quasi-static upper-body
contact under the GentleHumanoid spring compliance framework on the Unitree G1
(TRL~3--4). This is the most technically grounded contribution in the
rehabilitation literature, and it sets the engineering precondition for all
patient-contact rehabilitation studies that follow.

Two parallel lines of evidence establish why the compliance engineering matters.
Nguyen et al.~\cite{nguyen2024eeg} establish a neuroscientific rationale:
humanoid robot observation elicits sensorimotor EEG responses comparable to
human observation in healthy subjects, supporting an action observation therapy
(AOT) basis for stroke rehabilitation. Sobrepera et
al.~\cite{sobrepera2022feasibility} (n\,=\,6) document that clinical patients
preferred humanoid telepresence over standard video for neuromotor rehabilitation,
attributing this to physical embodiment (TRL~4). The convergence of three
distinct mechanisms---mirror neuron activation, nonverbal cueing via body
posture, and contact-safe compliance---points toward a coherent rehabilitation
use case, but the evidence is pre-clinical throughout.

The methodological limitations are sharp. The EEG study enrolled healthy
subjects only; replication with patients with motor impairment is required before
the AOT rationale translates to a therapeutic claim. The Sobrepera et al.\ case
series is the only study involving actual patients ($n\,=\,6$), and the platform
is not a full-size modern bipedal system. No standardized rehabilitation outcome
measure (Fugl-Meyer Assessment, 10-meter walk test) has been applied in any
humanoid rehabilitation study.

\subsection{Mental Health and Social Interaction}
\label{sec:mentalhealth}

The mental health evidence presents an inverse quality-specificity tradeoff that
is diagnostic of the field's current state. The two best-designed studies are
randomized: Robinson et al.~\cite{robinson2023brief} report a pilot RCT
($n\,=\,230$) in which an autonomous humanoid delivered a brief mindful breathing
intervention with positive wellbeing outcomes; Sayis and
Gunes~\cite{sayis2024technology} find that only the humanoid robot condition (not
a voice assistant) produced statistically significant mood improvements and
increased self-disclosure ($n\,=\,42$). Both results are credible. Neither,
however, specifies the humanoid model used, which prevents extrapolation to
full-size bipedal platforms. The platform-specific studies use weaker designs.
All studies involve subclinical or university wellness populations; none involves
a clinically diagnosed population under treatment. The embodiment advantage
is consistent across designs---but demonstrated on unspecified or out-of-scope
platforms, in non-clinical settings, with non-patient participants.

The implication for readiness assessment is precise: TRL~4--5 for social and
wellbeing applications in non-clinical settings (the RCT evidence supports this
claim); TRL~2--3 for bipedal humanoid deployment in clinical mental health
contexts (no qualifying evidence exists at this specificity). The LLM
personalization framework of Yuan et al.~\cite{yuan2025integrating}---which models
cognitive-emotional states probabilistically for dementia care---is directly
applicable to bipedal platforms, though the current implementation uses Pepper.
Methodological precedent for automated planning in pediatric clinical settings
exists~\cite{lindsay2022socially}. The next evidential requirement is an RCT
with a specified full-size bipedal platform in a clinical population.

\subsection{Hospital Logistics}
\label{sec:logistics}

No qualifying papers were found for humanoid-specific hospital logistics tasks
performed by bipedal humanoid robots. This absence reflects two structural
factors: wheeled autonomous mobile robots already address hospital logistics with
proven operational reliability, providing no demonstrated functional advantage for
the humanoid form factor in this task category; and academic research attention
in 2022--2026 was directed toward the more scientifically novel clinical procedure
and HRI domains. TRL for bipedal humanoid hospital logistics: 2 (technology
concept formulated; no demonstrated examples in relevant contexts).

\subsection{Cross-Domain Synthesis}
\label{sec:crossdomain}

Across the 14 qualifying papers, three structural patterns constrain the
generalizability of the current evidence base.

\textbf{Platform monoculture.} The Unitree G1 is the experimental platform in
4 of 14 papers (all three clinical procedure studies and the compliance
rehabilitation study). No other commercial platform appears in more than one
qualifying study. This monoculture is not incidental: as demonstrated in
Section~\ref{sec:accessibility}, the Unitree G1's open SDK is the primary
research access point for the commercial humanoid tier. Until equivalent SDK
access exists for higher-dexterity platforms (Sanctuary Phoenix Gen~8,
1X NEO Beta), cross-platform replication is structurally blocked.

\textbf{Geographic and institutional concentration.} Studies originate
predominantly from three countries (USA: 8 papers; Japan: 2; UK: 2; South Korea:
1; multinational EU: 1). The UC San Diego Yip Laboratory accounts for two of the
three clinical procedure papers; Johns Hopkins University contributes the third.
No study from a clinical institution in the Global South, Southeast Asia, or
continental Europe (excluding the SPRING program) appears in the qualifying set.
Healthcare workforce demographics, regulatory environments, and care delivery
models differ substantially across these absent geographies; findings from US and
Japanese studies may not transfer.

\textbf{Methodological gaps.} No qualifying study constitutes a randomized
controlled trial with a bipedal humanoid robot in a clinical population. No study
uses a validated clinical outcome measure as primary endpoint. No study exceeds a
single session or demonstration as its primary evaluation context. No study
includes health economic analysis or reports cost-per-outcome data. These gaps
collectively define the evidence requirements for any future regulatory submission:
a prospective, multi-session, clinically powered study on a full-size bipedal
humanoid in patient-contact conditions, with a validated clinical outcome measure
and a pre-registered analysis plan.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{2.1cm}p{2.1cm}p{2.0cm}p{1.4cm}p{1.1cm}l@{}}
    ...
    Hospital logistics & \multicolumn{4}{l}{\emph{No qualifying papers identified; wheeled autonomous systems dominate this role (TRL~2)}} & 2 \\
    \bottomrule
  \end{tabular}
\end{table}
...
actuarial absence. And some are circular: regulatory approval
...
deployment~\cite{topol2019high,rajpurkar2022ai}. No humanoid software stack has
...
\begin{figure}[htbp]
  ...
  \includegraphics[width=0.60\columnwidth]{figures/capability_radar.pdf}
...
The TRL scale captures technology maturity along a single axis. Clinical
deployment decisions require reasoning along at least three independent axes
simultaneously: \emph{autonomy level}, \emph{patient-contact intensity}, and
...
practitioner audiences. This subsection articulates those implications explicitly.
